...
Windows 用户优先从 GitHub Releases 下载打包好的 `DR2C-SaveEditor-*.exe`，无需安装 Python。
...
成品输出到 `release/DR2C-SaveEditor-1.0.1.exe`；PyInstaller 的构建缓存和隔离环境全部放在 `.temp/`。这些目录不会进入 Git。

## 安全与兼容性

活动存档混有 Deathforth 原始控制字节，不是普通 UTF-8 文本。本工具只对已识别字段做二进制定点替换，避免整份文件被文本编辑器重新编码。每次覆盖前会自动备份目标文件。

活动存档跨语言转换时，工具会主动重置已保存的“地区／路途随机事件牌堆”。这些堆里混有随语言包本地化的 Forth 词标识和富文本控制码，原样迁移会让目标语言版在读取中止，继而丢失后续天数和游戏模式。角色、物资、车辆、剩余天数、游戏模式与当前主流程不会被重置；目标版本会重新生成未来事件牌堆，因此少量已见事件可能再次出现。

这项兼容处理对英文 → 中文和中文 → 英文都生效。逐项映射并保留“已抽走／未抽走”的牌堆顺序尚未实现，原因和未来实现条件见 [TODO.md](TODO.md)。
