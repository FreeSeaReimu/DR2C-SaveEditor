...
成品输出到 `release/DR2C-SaveEditor-1.0.2.exe`；PyInstaller 的构建缓存和隔离环境全部放在 `.temp/`。这些目录不会进入 Git。
...
全局升级存档转换支持旧版向新版的前向兼容：若来源字段是目标字段的子集，会迁移全部同名字段，并将目标独有的新版数值字段归零。这样旧存档不会获得不属于它的新模式通关、PERK 或其他解锁；反向降级仍会安全阻止。
